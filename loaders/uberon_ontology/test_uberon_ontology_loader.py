...
import mock
import os
import uberon_ontology_loader


parameters = ["example_files/test.obo", "example_files/", "example_files/readme.md"]


@mock.patch.object(uberon_ontology_loader, 'parse_args')
def test_input_not_file_exists(args):
    with pytest.raises(SystemExit) as pytest_wrapped_e:
        args.return_value = ['file', parameters[1]]
        uberon_ontology_loader.main()
...
def test_number_of_lines(args):
    ...
    output_file = os.path.join(output_folder, f'go.json')

    lines_in_output = sum(1 for line in open(output_file))

    assert lines_in_output == 5
