...
def write_to_output(line, interactor_a_id_type, interactor_b_id_type, interactor_a_molecule_type_mi_id,
    ...
    if line[11] == "-":
        ...
        json_dictionary["interaction_types_mi_id"] = [int(line[11].split(":")[1].split("(")[0])]

    json_dictionary["source_database_mi_id"] = [int(line[12].split(":")[1].split('(')[0])]
    json_dictionary["source_database_mi_id"].append(923)
    ...
    if line[8] != "-":
        ...
        for pm_id in pm_ids:
            identifier = pm_id.split(":")[1]
            if int(identifier) not in json_dictionary["pmids"]:
                json_dictionary["pmids"].append(int(identifier))
...
def main():
    ...
    print(f'MESSSAGE [{strftime("%H:%M:%S")}]: iRefIndex Database Loader script finished successfully!')
