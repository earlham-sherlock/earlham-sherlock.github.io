...
import os
import json
import irefindex_db_loader


parameters = ["example_files/test.tsv", 9606, "18823568"]


@mock.patch.object(irefindex_db_loader, 'parse_args')
def test_input__file_exists(args):
    with pytest.raises(SystemExit) as pytest_wrapped_e:
        args.return_value = ['file', "uniprotac", "uniprotac", parameters[1], 326, "protein", 326, "protein", "0001",
                             "0915", False, parameters[2]]

        irefindex_db_loader.main()
...
def test_number_of_lines(args):
    args.return_value = [parameters[0], "uniprotac", "uniprotac", parameters[1], 326, "protein", 326, "protein", "0001",
                         "915", False, parameters[2]]
    ...
    output_file = os.path.join(new_folder, f'irefindex_db.json')
...
def test_values_in_the_output(args):
    ...
    with open(output_file, 'r') as out:
        ...
        for line in out:
            ...
            assert 18823568 in json_line["pmids"]
