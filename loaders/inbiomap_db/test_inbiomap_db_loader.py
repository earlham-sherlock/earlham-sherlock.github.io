...
import os
import json
import inbiomap_db_loader


parameters = ["example_files/test.tsv", 9606, "915", "27892958"]


@mock.patch.object(inbiomap_db_loader, 'parse_args')
def test_input__file_exists(args):
    with pytest.raises(SystemExit) as pytest_wrapped_e:
        args.return_value = ['file', "uniprotac", "uniprotac", parameters[1], 326, "protein", 326, "protein", False,
                             parameters[2], False, parameters[3]]
        inbiomap_db_loader.main()
...
def test_values_in_the_output(args):
    ...
    with open(output_file, 'r') as out:
        ...
        for line in out:
            ...
            assert json_line["interaction_types_mi_id"] == [915]
            assert 27892958 in json_line["pmids"]
